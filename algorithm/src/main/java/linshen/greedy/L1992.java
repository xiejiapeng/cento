...
public class L1992 {
            ...
            for (int j = 0; j < land[i].length; j++){
                if(land[i][j] == 1) {
                    if((i-1<0 || land[i-1][j] == 0) && (j-1<0 || land[i][j-1] == 0)) {
                        int ml = i;
                        int md = j;
                        while (md < land[i].length && land[i][md] == 1) {
                            md++;
                        }
                        while (ml < land.length && land[ml][j] == 1){
                            ml++;
                        }
                        ans.add(new int[]{i,j,ml-1,md-1});
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        System.out.println(new L1992().findFarmland(new int[][]{{1,1},{1,1}}));
    }
}
